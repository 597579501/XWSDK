...
#import "XWStoreKeychainPersistence.h"
#import <Security/Security.h>

NSString* const XWStoreTransactionsKeychainKey = @"XWStoreTransactions";
...
void RMKeychainSetValue(NSData *value, NSString *key)
{
    ...
    {
        NSLog(@"StoreKeychainPersistence: failed to set key %@ with error %ld.", key, (long)status);
    }
}
...
NSData* RMKeychainGetValue(NSString *key)
{
    ...
    {
        NSLog(@"StoreKeychainPersistence: failed to get key %@ with error %ld.", key, (long)status);
    }
    return (__bridge NSData*)value;
}

@implementation XWStoreKeychainPersistence {
...
- (NSDictionary*)transactionsDictionary
{
    ...
    { // Reading the keychain is slow so we cache its values in memory
        NSData *data = RMKeychainGetValue(XWStoreTransactionsKeychainKey);
        ...
        if (data)
        {
            ...
            if (!transactions)
            {
                NSLog(@"StoreKeychainPersistence: failed to read JSON data with error %@", error);
            }
        }
...
- (void)setTransactionsDictionary:(NSDictionary*)dictionary
{
    ...
    if (dictionary)
    {
        ...
        if (!data)
        {
            NSLog(@"StoreKeychainPersistence: failed to write JSON data with error %@", error);
        }
    }
    RMKeychainSetValue(data, XWStoreTransactionsKeychainKey);
}
