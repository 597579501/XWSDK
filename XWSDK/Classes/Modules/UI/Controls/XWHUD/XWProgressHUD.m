...
#ifndef kCFCoreFoundationVersionNuXWer_iOS_7_0
    #define kCFCoreFoundationVersionNuXWer_iOS_7_0 847.20
#endif

#ifndef kCFCoreFoundationVersionNuXWer_iOS_8_0
    #define kCFCoreFoundationVersionNuXWer_iOS_8_0 1129.15
#endif

#define XWMainThreadAssert() NSAssert([NSThread isMainThread], @"XWProgressHUD needs to be accessed on the main thread.");

CGFloat const XWProgressMaxOffset = 1000000.f;

static const CGFloat XWDefaultPadding = 4.f;
static const CGFloat XWDefaultLabelFontSize = 16.f;
static const CGFloat XWDefaultDetailsLabelFontSize = 12.f;


@interface XWProgressHUD () {
    // Deprecated
...
@interface XWProgressHUDRoundedButton : UIButton
@end
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    XWProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (XWProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (XWProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = XWProgressHUDAnimationFade;
    _mode = XWProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
    _defaultMotionEffectsEnabled = YES;

    // Default color, depending on the current iOS version
    BOOL isLegacy = kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuXWer_iOS_7_0;
...
- (void)animateIn:(BOOL)animatingIn withType:(XWProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == XWProgressHUDAnimationZoom) {
        type = animatingIn ? XWProgressHUDAnimationZoomIn : XWProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == XWProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == XWProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == XWProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == XWProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
#pragma clang diagnostic pop
    ...
    if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuXWer_iOS_7_0) {
...
- (void)done {
    ...
    XWProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<XWProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    XWBackgroundView *backgroundView = [[XWBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = XWProgressHUDBackgroundStyleSolidColor;
    ...
    XWBackgroundView *bezelView = [XWBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:XWDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:XWDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [XWProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:XWDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[XWRoundProgressView class]];

    XWProgressHUDMode mode = self.mode;
    if (mode == XWProgressHUDModeIndeterminate) {
    ...
    else if (mode == XWProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[XWBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == XWProgressHUDModeDeterminate || mode == XWProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[XWRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == XWProgressHUDModeAnnularDeterminate) {
            [(XWRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == XWProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == XWProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[XWProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[XWProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[XWRoundProgressView class]]) {
        XWRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [XWRoundProgressView appearanceWhenContainedIn:[XWProgressHUD class], nil];
#else
        appearance = [XWRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[XWProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((XWRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((XWRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[XWBarProgressView class]]) {
        XWBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [XWBarProgressView appearanceWhenContainedIn:[XWProgressHUD class], nil];
#else
        appearance = [XWBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[XWProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((XWBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((XWBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    XWBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoXWie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? XWDefaultPadding : 0.f;
...
- (void)setMode:(XWProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == XWProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    BOOL iOS8OrLater = kCFCoreFoundationVersionNuXWer >= kCFCoreFoundationVersionNuXWer_iOS_8_0;
...
@implementation XWRoundProgressView
...
- (void)drawRect:(CGRect)rect {
    ...
    BOOL isPreiOS7 = kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuXWer_iOS_7_0;
...
@implementation XWBarProgressView
...
@implementation XWBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuXWer_iOS_7_0) {
            _style = XWProgressHUDBackgroundStyleBlur;
            if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuXWer_iOS_8_0) {
                _color = [UIColor colorWithWhite:0.8f alpha:0.6f];
            } else {
                _color = [UIColor colorWithWhite:0.95f alpha:0.6f];
            }
        } else {
            _style = XWProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(XWProgressHUDBackgroundStyle)style {
    if (style == XWProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNuXWer_iOS_7_0) {
        style = XWProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    XWProgressHUDBackgroundStyle style = self.style;
    if (style == XWProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuXWer_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == XWProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNuXWer_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation XWProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [XWProgressHUD allHUDsForView:view];
    for (XWProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable XWProgressHUDCompletionBlock)completion {
...
- (BOOL)diXWackground {
    XWBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == XWProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDiXWackground:(BOOL)diXWackground {
    XWMainThreadAssert();
    self.backgroundView.style = XWProgressHUDBackgroundStyleSolidColor;
    self.backgroundView.color = diXWackground ? [UIColor colorWithWhite:0.f alpha:.2f] : [UIColor clearColor];
}
...
@implementation XWProgressHUDRoundedButton
